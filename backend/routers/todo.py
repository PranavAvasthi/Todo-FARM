...
async def get_all_todos(user_id: str = Depends(get_current_user)):
    todos = await db.todos.find({"user_id": user_id}).to_list(100)
    return todos

@router.get("/todos/{todo_id}", response_model = TodoResponse)
async def get_todo_by_id(todo_id: str, user_id: str = Depends(get_current_user)):
    try:
        todo = await db.todos.find_one({"_id": ObjectId(todo_id), "user_id": user_id})
    except Exception as e:
...
async def update_todo(todo_id: str, todo_update: TodoUpdate, user_id: str = Depends(get_current_user)):
    ...
    result = await db.todos.update_one({"_id": ObjectId(todo_id), "user_id": user_id}, {"$set": todo_dict})
    
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="Todo not found")
    
    updated_todo = await db.todos.find_one({"_id": ObjectId(todo_id), "user_id": user_id})
...
async def update_todo_helper(todo_id: str, new_completed: bool, user_id: str):
    ...
    parent_todo = await db.todos.find_one({"_id": ObjectId(todo_id), "user_id": user_id})
...
async def delete_todo(todo_id: str, user_id: str = Depends(get_current_user)):
    todo_to_delete = await db.todos.find_one({"_id": ObjectId(todo_id), "user_id": user_id})
...
async def delete_todo_helper(todo_id: str, user_id: str):
    current_todo = await db.todos.find_one({"_id": ObjectId(todo_id), "user_id": user_id})
